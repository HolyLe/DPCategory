...
@interface DPControlTarget : NSObject
...
@implementation DPControlTarget
...
@implementation UIControl (DPCategory)

- (void)addEventBlock:(controlTargeAction)block forEvents:(UIControlEvents)events{
    if (!events) return;
    if (!block) return;
    DPControlTarget *target = [[DPControlTarget alloc] initWithBlock:block events:events];
    [self addTarget:target action:@selector(sendControl:) forControlEvents:events];
    NSMutableArray *targets = [self dp_controlTargetsArray];
...
- (void)removeAllEvents{
    ...
    [[self dp_controlTargetsArray] removeAllObjects];
    [[self dp_controlTargetsDic] removeAllObjects];
}
...
- (BOOL)containsEventBlockForKey:(NSString *)key{
    NSMutableDictionary *dic = [self dp_controlTargetsDic];
...
- (void)addEventBlock:(controlTargeAction)block forEvents:(UIControlEvents)events ForKey:(NSString *)key{
    ...
    DPControlTarget *target = [[DPControlTarget alloc] initWithBlock:block events:events];
    [self addTarget:target action:@selector(sendControl:) forControlEvents:events];
    [[self dp_controlTargetsDic] setObject:target forKey:key];
}

- (void)removeEventBlockForKey:(NSString *)key event:(UIControlEvents)events{
    NSMutableDictionary *dic = [self dp_controlTargetsDic];
    DPControlTarget *target = [dic objectForKey:key];
    if (target) {
...
- (void)removeAllEventBlocksForEvents:(UIControlEvents)controlEvents{
    ...
    for (DPControlTarget *target in targets) {
    ...
    NSMutableDictionary *targetsDic = [self dp_controlTargetsDic];
    [targetsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, DPControlTarget * _Nonnull target, BOOL * _Nonnull stop) {
        if (target.events & controlEvents) {
            UIControlEvents newEVent = target.events & (~controlEvents);
            if (newEVent) {
                [self removeTarget:target action:@selector(sendControl:) forControlEvents:target.events];
...
- (NSMutableArray <DPControlTarget *>*)dp_controlTargetsArray{
...
- (NSMutableDictionary <NSString * ,DPControlTarget *>*)dp_controlTargetsDic{
