...
@implementation UIDevice (DPCategory)
...
static uint64_t _net_counter_get_by_type(net_interface_counter *counter, DPNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & DPNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & DPNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & DPNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & DPNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & DPNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & DPNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}
